...
export default function AdmissionTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://127.0.0.1:8000/admission", {
        ...
        console.log(
          "Fetching admission from database successful!!!",
          response.data
        );
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/admission/${id}`)
      .then((res) => {
    ...
    <div className="datatable">
      <div className="datatableTitle">
        ...
        <div className="search">
          <input
            type="text"
            placeholder="Search admissions table..."
            value={searchTerm}
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <AdmissionEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <AdmissionView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
