...
import "./bedTable.scss";
import SearchOutlinedIcon from "@mui/icons-material/SearchOutlined";
import { Button } from "@mui/material";

import BedForm from "../bed/BedForm";
import BedEdit from "../bed/BedEdit";
import BedView from "../bed/BedView";
...
export default function BedTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://127.0.0.1:8000/bed", {
        ...
        console.log("Fetching beds from database successful!!!", response.data);
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/bed/${id}`)
      .then((res) => {
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <BedEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <BedView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
