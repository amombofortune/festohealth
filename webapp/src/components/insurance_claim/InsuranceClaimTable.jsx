...
import "./insuranceClaimTable.scss";
import SearchOutlinedIcon from "@mui/icons-material/SearchOutlined";
import { Button } from "@mui/material";

import InsuranceClaimForm from "../insurance_claim/InsuranceClaimForm";
import InsuranceClaimView from "../insurance_claim/InsuranceClaimView";
import InsuranceClaimEdit from "../insurance_claim/InsuranceClaimEdit";
...
export default function InsuranceClaimTable() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8000/insurance_claim",
          {
        ...
        console.log(
          "Fetching insurance claim from database successful!!!",
          response.data
        );
        setData(response.data);
      } catch (error) {
        console.error("Failed to fetch insurance claim data:", error);
        // Handle error fetching insurance claim data
      }
    };
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/insurance_claim/${id}`)
      .then((res) => {
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <InsuranceClaimEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <InsuranceClaimView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
