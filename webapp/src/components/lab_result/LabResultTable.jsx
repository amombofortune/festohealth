...
import LabResultForm from "../lab_result/LabResultForm";
import LabResultEdit from "../lab_result/LabResultEdit";
import LabResultView from "../lab_result/LabResultView";
...
export default function Trial() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8000/lab_test_result",
          {
        ...
        console.log(
          "Fetching lab result from database successful!!!",
          response.data
        );
        setData(response.data);
      } catch (error) {
        console.error("Failed to fetch lab result data:", error);
        // Handle error fetching lab result data
      }
    };
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/lab_test_result/${id}`)
      .then((res) => {
    ...
    <div className="datatable">
      <div className="datatableTitle">
        ...
        <div className="search">
          <input
            type="text"
            placeholder="Search lab results table..."
            value={searchTerm}
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <LabResultEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <LabResultView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
