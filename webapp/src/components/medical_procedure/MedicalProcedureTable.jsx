...
import "./medicalProcedureTable.scss";
import SearchOutlinedIcon from "@mui/icons-material/SearchOutlined";
import { Button } from "@mui/material";

import MedicalProcedureEdit from "../medical_procedure/MedicalProcedureEdit";
import MedicalProcedureForm from "../medical_procedure/MedicalProcedureForm";
import MedicalProcedureView from "../medical_procedure/MedicalProcedureView";
...
export default function MedicalProcedureTable() {
  const columns = [
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8000/medical_procedure",
          {
        ...
        console.log(
          "Fetching medical procedure from database successful!!!",
          response.data
        );
        setData(response.data);
      } catch (error) {
        console.error("Failed to fetch medical procedure data:", error);
        // Handle error fetching medical procedure data
      }
    };
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/medical_procedure/${id}`)
      .then((res) => {
    ...
    <div className="datatable">
      <div className="datatableTitle">
        ...
        <div className="search">
          <input
            type="text"
            placeholder="Search medical procedure table..."
            value={searchTerm}
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <MedicalProcedureEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <MedicalProcedureView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
