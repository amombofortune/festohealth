...
import "./patientConsentTable.scss";
import SearchOutlinedIcon from "@mui/icons-material/SearchOutlined";
import { Button } from "@mui/material";

import PatientConsentForm from "../patient_consent/PatientConsentForm";
import PatientConsentEdit from "../patient_consent/PatientConsentEdit";
import PatientConsentView from "../patient_consent/PatientConsentView";
...
export default function PatientConsentTable() {
  const columns = [
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8000/patient_consent",
          {
        ...
        console.log(
          "Fetching patient consent from database successful!!!",
          response.data
        );
        setData(response.data);
      } catch (error) {
        console.error("Failed to fetch patient consent data:", error);
        // Handle error fetching patient consent data
      }
    };
  ...
  const handleDelete = async (id, e) => {
    ...
    await axios
      .delete(`http://127.0.0.1:8000/patient_consent/${id}`)
      .then((res) => {
    ...
    <div className="datatable">
      <div className="datatableTitle">
        ...
        <div className="search">
          <input
            type="text"
            placeholder="Search patient consent table..."
            value={searchTerm}
      ...
      <div>
        {selectedRow && (
          <Modal open={openEditForm} onClose={handleCloseEditForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <PatientConsentEdit selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
      ...
      <div>
        {selectedRow && (
          <Modal open={openViewForm} onClose={handleCloseViewForm}>
            <Box style={modalStyle}>
              <Box sx={{ width: "100%" }}>
                <PatientConsentView selectedRow={selectedRow} />
              </Box>
            </Box>
          </Modal>
        )}
